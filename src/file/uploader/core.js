import { FileUploadStatus, UploaderErrorMap, } from "./type/index.js";
/**
 * 文件上传核心代码
 * 使用封装后的文件类型,
...
export class FileUploader {
    ...
    async doUpload(file, params) {
        ...
        params
            ? Object.entries(params).forEach((item) => {
                item[1] ? formData.append(item[0], item[1]) : null;
            })
            : null;
        ...
        if (res && res.fileId) {
            ...
            this.EmitError(UploaderErrorMap.UploadError, `${file.name}上传失败`, file);
        }
        this.afterUpload();
        return { file, res };
    }
    ...
    /** ANCHOR 文件状态变化 回调 */
    onFileStatueChangedCallback = undefined;
    onFileStatusChanged(callback) {
        this.onFileStatueChangedCallback = callback;
    }
    /** ANCHOR 文件上传完成的回调 */
    onFileUploadedCallback = undefined;
    /** 设置文件上传完成的回调 */
    onFileUploaded(callback) {
        this.onFileUploadedCallback = callback;
    }
    /** ANCHOR 文件百分比变化 回调 */
    onFilePercentChangedCallback = undefined;
    onFilePercentChanged(callback) {
        this.onFilePercentChangedCallback = callback;
    }
    /** ANCHOR 文件状态变化 回调 */
    onFileUploadErrorCallback = undefined;
